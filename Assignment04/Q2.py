...
def laplacian(img,threshold):
    ...
    result = np.absolute(convolve2d(img,L,'same'))

    mx,mn = np.max(result),np.min(result)
    result = 255*(result-mn)/(mx-mn)

    print(result[:10,:10])
    plt.imshow(result,cmap='gist_gray_r')
    plt.show()
...
if __name__=='__main__':
    ...
    img = imread(file)
    img = img[230:250,230:250]
    plt.subplot(131)
    plt.imshow(img,'gray')
    result = laplace(img,ksize=5)
    plt.subplot(132)
    plt.imshow(result,'gray')
    freq = np.array([[0,-1,0],[-1,4,-1],[0,-1,0]])
    freq = np.fft.fft2(result)
    freq = np.fft.fftshift(freq)
    freq = np.log(1+np.absolute(freq))
    plt.subplot(133)
